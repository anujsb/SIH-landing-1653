...
import { Link } from "react-router-dom"; // Correct React Router Link import
    ...
    <AnimatePresence mode="wait">
      <div className="flex items-center justify-center">
        <motion.div
          ...
          className={`flex justify-between sm:mx-5 md:mx-10 lg:mx-10 w-max-[1120px] fixed top-10 border border-transparent rounded-2xl bg-sec-col shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-[5000] px-5 py-2 items-center backdrop-blur-lg ${className}`}
        >
          <div className="mr-3">
            <img src={logo} alt="EACC.dev logo" className="h-10 w-30 mr-2 " />
          </div>
          <div className="hidden sm:flex space-x-4 text-[#404040] hover:text-black mx-20">
            {navItems.map((navItem, idx) => (
              <Link
                key={`link=${idx}`}
                to={navItem.link}
                className="relative items-center flex space-x-4 text-[#404040] hover:text-black"
              >
